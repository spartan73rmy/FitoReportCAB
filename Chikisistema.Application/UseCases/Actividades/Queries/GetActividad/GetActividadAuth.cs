...
{
    public class GetActividadAuth : IAuthenticatedRequest<GetActividadQuery, GetActividadResponse>
    {
        private readonly IChikisistemaDbContext db;
        private readonly IUserAccessor currentUser;

        public GetActividadAuth(IChikisistemaDbContext db, IUserAccessor currentUser)
        {
            this.db = db;
            this.currentUser = currentUser;
        }

        public async Task Validate(GetActividadQuery request, ValidationResult validationResult)
        {
            var curso = await db
                .ActividadCurso
                .Select(el => new
